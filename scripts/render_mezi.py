#!/usr/bin/env python3
"""Mike: clean classroom, livelier teach gestures, lips."""
...
def draw_classroom(topic: str, definition: str = "", cta: str = "") -> Image.Image:
    ...
    d.rectangle([0, int(bh * 0.7) - 8, bw, int(bh * 0.7)], fill=ACCENT)
    for i in range(2):
        x0 = 36 + i * 150
        d.rounded_rectangle([x0, 40, x0 + 130, 200], 10, fill=(190, 210, 230))
    bx0, by0, bx1, by1 = 50, 230, bw - 50, int(bh * 0.58)
...
def make_bg(topic: str = "", definition: str = "", cta: str = "") -> Image.Image:
...
def cam_for(move, p):
    if move in ("point", "explain", "count"):
        return "pan"
    if move == "sit":
        return "close"
    if move == "present":
...
def body_for(move, t, blink):
    if blink and move in ("talk", "welcome", "present", "happy", "question", "explain", "lean"):
...
def composite_host(move, mouth_open, blink, t):
...
def draw_chair(frame, cx, seat_y):
...
def word_windows(text, duration):
    ...
    if not words:
        return [(0, duration, ["..."], 0)]
    n, slot, windows, i = len(words), duration / max(len(words), 1), [], 0
    while i < n:
        chunk = words[i : i + 5]
...
def draw_ui(rgb, text, t, duration, topic, dyk, cta, p):
...
def main():
    ...
    bg = make_bg(topic, definition, cta)
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for i in range(n):
            ...
            char = composite_host(move, open_at(cues, t), blink, t)
