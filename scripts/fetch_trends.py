#!/usr/bin/env python3
"""Study/exam trends only → data/trending_topics.json (TOS-safe)."""
    ...
    for seed in ["Photosynthesis", "Electric current", "Quadratic equation", "Osmosis", "Mitosis"]:
        try:
            ...
            if isinstance(data, list) and len(data) > 1:
                for title in data[1]:
                    t = clean_title(title) or (title if is_school_safe(title) else None)
                    if t:
                        found.append(t)
            time.sleep(0.3)
        except Exception as e:
...
def main() -> None:
    ...
    titles = filter_title_list(dedupe(titles))[:40]
